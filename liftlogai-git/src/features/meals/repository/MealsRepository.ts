...
import { allFoods, toLocalFood } from '../data/foodDatabase';

type MealWithoutItems = Omit<Meal, 'items'>;

export class MealsRepository {
  /**
   * One-time (idempotent) migration for meal items that were stored with
   * per-100g nutrition values instead of per-serving values.
   *
   * Before the toLocalFood() fix, bodybuilding/supplement foods were exposed
   * to the regular search with per-100g macros (e.g. whey 400 kcal / P80),
   * and createMealItemFromSearchResult stored them verbatim. Those legacy
   * items now over-report calories/macros.
   *
   * Detection: an item whose name matches a RawFoodEntry, whose servingSize
   * equals the entry's defaultServing, and whose stored calories equal the
   * entry's per-100g calories is treated as legacy per-100g storage and
   * rewritten to the correct per-serving values via toLocalFood().
   *
   * Safe to run repeatedly: after rewriting, stored calories no longer equal
   * per-100g calories, so the item is skipped on subsequent runs.
   */
  static async migrateLegacyMealItemNutrition(): Promise<void> {
    try {
      const items = await db.mealItems.toArray();
      if (items.length === 0) return;

      const updates: { key: string; changes: Partial<MealItem> }[] = [];
      const foodByExactName = new Map(
        allFoods.map((f) => [f.name, f] as const)
      );

      for (const item of items) {
        const food = foodByExactName.get(item.name);
        if (!food) continue;
        if (item.servingSize !== food.defaultServing) continue;

        const per100gCalories = food.nutritionPer100g.calories;
        // Only rewrite if the stored value matches the legacy per-100g storage
        // (this also guarantees idempotency).
        if (item.calories !== per100gCalories) continue;

        const serving = toLocalFood(food);
        updates.push({
          key: item.id,
          changes: {
      ...
      await db.transaction('rw', db.mealItems, async () => {
        for (const u of updates) {
          await db.mealItems.update(u.key, u.changes);
        }
      });
    } catch (err) {
      // Migration must never break the meals page.
      console.error('Error migrating legacy meal item nutrition:', err);
    }
  }
